...
import { EMPRESA_STATUS_LABEL, REGIME_LABEL, DOC_TIPO_LABEL, DOC_STATUS_LABEL, formatCompetencia, formatCNPJ, competenciaAtual, mesAnoParaCompetencia } from "@/lib/format";
...
function ClienteDetalhe() {
  ...
  // Selecionar Jan/2026 filtra 2026-01 — sem offset "mês de trabalho - 1".
  const [compPadrao] = useState(competenciaAtual());
  const [mes, setMes] = useState<number>(Number(compPadrao.slice(5, 7)));
  const [ano, setAno] = useState<number>(Number(compPadrao.slice(0, 4)));
  const competencia = mesAnoParaCompetencia(ano, mes);
  const anosDisponiveis = [ano, ano - 1, ano - 2, ano - 3].filter((v, i, a) => a.indexOf(v) === i);
      ...
      <div className="flex items-end justify-between mb-6 gap-4">
        ...
        <div className="flex items-end gap-3">
          <div className="flex flex-col items-end gap-1">
            <span className="text-[11px] uppercase tracking-wide text-muted-foreground">Competência {formatCompetencia(competencia)}</span>
